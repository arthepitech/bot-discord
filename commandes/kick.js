...
module.exports = {
    ...
    async run(bot, message, args) {
        ...
        if (message.user.id === user.id) {
            return message.reply("Essaie pas de te kick !")
        }
        if ((await message.guild.fetchOwner()).id === user.id) {
            return message.reply("Pas de kick du proprio.")
        }
        ...
        if (member && message.member.roles.highest.comparePositionTo(member.roles.highest) <= 0) {
            return message.reply("Tu ne peux pas kick des gens au même échelon que toi.")
        }
        try {
            await user.send(`Tu as été kick du server ${message.guild.name} par ${message.user.tag} pour la raison: \`${reason}\``)
        } catch(err) {}
